...
def date_range(today): #NON CUMULATIVE DATES
    if today.month >= 7 and today.month <= 9:
        ...
        date_range_start = datetime.date(today.year, 10, 1) #Second Quarter Only
        date_range_end = datetime.date(today.year, 12, 31)
        FY = today.year
        FQ = 'Q2'
...
df['Agency'] = df.apply(agency_mapping, axis = 1)
...
if len(df['ContractID'][~df['ContractID'].isnull()].unique()) <=1000:
    ...
    doc_hdr = pd.read_table(int_data_path+'\\'+r'doc_hdr.txt', header=0, sep=',', dtype={'PO_REPL_DEPT_CD': str, 'PO_REPL_ID': str})

else:
    print ('More than 1000 Contracts. Please Investigate')
...
join2['OrigContractID'] = join2.apply(remove_nan, axis = 1)
join2 = join2.drop_duplicates(subset = ['ContractID', 'OrigContractID'])

tup = str(tuple(join2['OrigContractID'].unique()[0:1000]))
tup = tup.replace('\'','\\\'')
tup = tup.replace('u', '')
...
join3 = join3.drop_duplicates(subset = ['ContractID_x', 'OrigContractID'], keep = 'first')
...
###Tuple for Registration Date doc_hdr sql pull
###substitute this into fms_lookup5.py

tup = str(tuple(join5['OrigContractID'].unique()))
tup = tup.replace('\'','\\\'')
tup = tup.replace('u', '')
...
join6['OrigContractID'] = join6.apply(remove_nan, axis = 1)
...
df = join6
df = df[[u'Agency', u'ContractID_x', u'VendorName', u'Purpose',u'Method', u'Industry', u'RegistrationDate', u'ContractValue', u'OrigContractID', u'AwdMethodName', u'REG_DT', u'DOC_DEPT_CD_x']]
df.columns = ['Agency', 'ContractID', 'VendorName', 'Purpose', 'Method', 'Industry', 'RegistrationDate', 'ContractValue', 'OrigContractID', 'AwdMethodName', 'REG_DT', 'DOC_DEPT_CD']

nypd_contracts = df[df['Agency'] == 'NYPD']['ContractID'].tolist()
nypd_contracts = tuple(nypd_contracts)
nypd_contracts = str(nypd_contracts)
nypd_contracts = nypd_contracts.replace('u\'','\'')
nypd_contract = nypd_contracts.replace('\'','\\\'')
...
newpath = r'S:\Contracts\Research and IT\08 - MWBE\DAS Only\09 - Python and R Scripts\Development\LL1 TADs Creation Outbound\Outbound TADs Creation\TADs_Holding\FY%s_%s_%s' %(str(FY),str(FQ),str(t))
