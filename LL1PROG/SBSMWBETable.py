...
def reportcategory_function(row):  # qryCertMWBE_02b_Update_RepCat
    if row['MWBEType'] == 'MBE' and row['EthGen'] == 'Asian American':
        return 'Male-Owned MBE - Asian'
    elif row['MWBEType'] == 'MBE' and row['EthGen'] == 'Black American':
        return 'Male-Owned MBE - Black'
    elif row['MWBEType'] == 'MBE' and row['EthGen'] == 'Hispanic American':
        return 'Male-Owned MBE - Hispanic'
    elif row['MWBEType'] == 'MBE' and row['EthGen'] == 'Asian-Indian':
        return 'Male-Owned MBE - Hispanic'
    elif row['MWBEType'] == 'WBE':
        return 'WBE - Caucasian Woman'
    elif row['MWBEType'] in ['M/WBE', 'MWBE'] and row['EthGen'] == 'Asian American':
        return 'WBE - Asian'
    elif row['MWBEType'] in ['M/WBE', 'MWBE'] and row['EthGen'] == 'Black American':
        return 'WBE - Black'
    elif row['MWBEType'] in ['M/WBE', 'MWBE'] and row['EthGen'] == 'Hispanic American':
        return 'WBE - Hispanic'
...
# os.system('python S:\Contracts\\\\\"Research and IT\"\\\\\"08 - MWBE\"\\\\\"DAS Only\"\\\\\"09 - Python and R Scripts\"\\\LL1ProgFY19Q3\\\\\"SBS MWBE List to Access\"\\\\R_VEND_CUST.py')
...
def ethnicity_clean(row):  # qryCertMWBE_02b_Update_RepCat
    if 'ian' in str(row['ETHNICITY_x']).lower() and 'ific' in str(row['ETHNICITY_x']).lower():
        return 'Asian Pacific'
    elif 'ian' in str(row['ETHNICITY_x']).lower() and 'ian' in str(row['ETHNICITY_x']).lower():
        return 'Asian Indian'
    elif 'panic' in str(row['ETHNICITY_x']).lower():
        return 'Hispanic'
    elif 'rity' in str(row['ETHNICITY_x']).lower():
        return 'Non-Minority'
    elif 'ack' in str(row['ETHNICITY_x']).lower():
        return 'Black'
